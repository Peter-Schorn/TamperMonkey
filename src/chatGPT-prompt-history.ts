...
(function (): void {
    ...
    let promptTextArea: HTMLElement | null = null;

    let promptTextObserver: MutationObserver | null = null;

    /** Gets all of the text in the prompt text area. */
    function getPromptTextAreaText(): string {
    ...
    function handleKeyDown(e: KeyboardEvent): void {
        ...
        console.log(
            `handleKeyDown: currentPromptText: \`${currentPromptText}\``
        );
    ...
    function observePromptTextChanges(): void {
    ...
    function updateUserMessages(): void {
        ...
        const userMessageBubbles = document.querySelectorAll(
            ".user-message-bubble-color"
        );
        const messages = Array.from(userMessageBubbles).map((
    ...
    function registerUpdateUserMessagesObserver(): void {
        const selector = "#thread #page-header + div";
        const threadElement = document.querySelector(selector);
        if (!threadElement) {
            console.error(
                "registerUpdateUserMessagesObserver: threadElement is null"
            );
            return;
        }

        const scrollToBottomButtonSelector =
            ".cursor-pointer.absolute.z-30.rounded-full";
            ...
            if (mutations.every(mutation => {
                ...
                if (target instanceof Element) {
                    if (
                        ...
                        console.log(
                            "threadObserver: ignoring mutation to prompt text area"
                        );
                        return true;
                    }
                }
                ...
                if (modifiedNodes.every(node => {
                    if (node instanceof Element) {
                        if (
                            node.querySelector(scrollToBottomButtonSelector) ||
                    ...
                    console.log(
                        "threadObserver: ignoring mutations to scroll to bottom button"
                    );
                    return true;
                }
                ...
                console.log(
                    "threadObserver: ignoring mutations"
                );
                return;
            }
    ...
    const extendedWindow = window as unknown as ChatGPTTypes.Window;

    extendedWindow.getCurrentPromptText = (): string => {
        return currentPromptText;
    };

    extendedWindow.getUserMessages = (): string[] => {
        return userMessages;
    };

    extendedWindow.updateUserMessages = (): void => {
        updateUserMessages();
    };
