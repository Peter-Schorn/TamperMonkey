...
/**
 * Stores recently used barcode IDs with their timestamps to prevent duplicate
 * insertions within a certain time window.
 *
 * The keys are barcode IDs and the values are the timestamps (in unix time) of
 * when the barcode was last used.
 */
type BarcodeStore = Record<string, number>;
...
(function (): void {
    ...
    let USER = GM_getValue<string | null>(USERNAME_KEY, null);
    ...
    const TIME_TO_LIVE_MS = 30_000; // 30 seconds
     ...
     */
    function tryUseBarcodeID(barcodeID: string): boolean {
        const now = Date.now();

        const store = GM_getValue<BarcodeStore>(RECENT_BARCODES_KEY, {});
     ...
     */
    function scannedBarcodesReviver(key: string, value: unknown): unknown {
    ...
    function appendToElement(
        ...
        try {
            if (
                ...
                console.log(
                    "will append to element:", element.outerHTML,
                    element
                );
                console.log(
     ...
     */
    function appendToActiveElement(text: string): void {
        ...
        if (
            ...
            ] as Element[]).includes(element)
        ) {
            // do not recurse through the entire document
            console.warn(
                "active element is body or documentElement; not appending."
            );
            return;
        }
     ...
     */
    function fetchLatestBarcode(): void {
        void (async (): Promise<void> => {
            try {
                ...
                if (!response.ok) {
                    ...
                    console.error(
                        `Failed to fetch latest barcode: ${response.status} ` +
                ...
                ) as ScannedBarcodesResponse<Date>;
                ...
                if (!startConnectionAttemptDate) {
                    ...
                    console.log(
                        "ignoring fetched barcodes because web socket " +
                ...
                if (!pageIsActive()) {
                    console.log(
                        "ignoring fetched barcodes because page not active"
                    );
                    return;
                }
                ...
                const toleranceMS = 5_000; // 5 seconds
                ...
                if (scanDate.getTime() >= cutoffTime) {
                    console.log(
                        "latest barcode was scanned within tolerance; " +
                    ...
                    console.log(
                        `latest barcode "${latestBarcode.barcode}" scanned ` +
    ...
    function onPageVisible(): void {
        ...
        if (!USER) {
            console.warn(
                "no BarcodeDrop username provided; not starting web socket " +
    ...
    GM_addValueChangeListener(
        USERNAME_KEY,
        (_name, oldValue, newValue, remote) => {
            if (USER === newValue) {
                // if the value didn't actually change, do nothing
                return;
            }
            console.log(
                `username changed from "${oldValue}" to "${newValue}" ` +
                `(remote: ${remote})`
            );
            USER = newValue;
