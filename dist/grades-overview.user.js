...
(function () {
    ...
    function updateOverallGrades() {
        try {
            // high grade color class: excellent
            const overallGradePillSelector = 'bb-display-grade-pill[grade="baseGradesStudent.externalGrade"]';
            // .grades-list.current-term
            const gradesContainer = document.querySelector(".grades-list.current-term");
            // MARK: Beginning Web Programming: Overall Grade
            const webProgCard = gradesContainer.querySelector("#card__949284_1");
            // fix overall grade
            const webProgOverallGradePill = webProgCard.querySelector(overallGradePillSelector);
            const webProgOverallGradePillColorDiv = webProgOverallGradePill.querySelector(".customGradePill");
            ...
            for (const assessment of webProgAssessments) {
                try {
                    const titleContainer = assessment.querySelector(".element-details");
                    if (!titleContainer) {
                        continue;
                    }
                    ...
                    if (titleText.includes("Exam 2")) {
                        ...
                        const gradePill = assessment.querySelector("bb-display-grade-pill");
                        const gradePillColorDiv = gradePill.querySelector(".customGradePill");
                        const gradeTextElement = gradePillColorDiv.querySelector("bdi");
                    ...
                    console.error("updateOverallGrades: error processing assessment:", error);
                }
            }
        }
        catch (error) {
